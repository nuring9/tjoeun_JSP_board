...
<body>
	...
	<div class="container mt-5">
				...
				<table class="table table-hover text-center align-middle mb-0">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-start">
								<%-- 제목 클릭 시 상세 페이지 이동 --%> <a
								href="board?action=view&id=<%=dto.getId()%>"
								class="d-block text-decoration-none text-dark fw-bold"> <%=dto.getTitle()%>
							</a>
							</td>
							<td><%=dto.getWriter()%></td>
							<td><%=dto.getRegdate()%></td>
							<td><a href="board?action=delete&id=<%=dto.getId()%>"
		...
		<div class="d-flex justify-content-center mt-4 gap-2 flex-wrap">
			...
			<a href="board?action=list&page=<%=i%>"
				class="btn btn-sm btn-light border-dark
       <%=(i == currentPage) ? "fw-bold" : ""%>">
				<%=i%>
			</a>
